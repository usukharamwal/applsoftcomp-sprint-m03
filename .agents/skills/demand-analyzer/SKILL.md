...
description: Analyze demand patterns from CSV/Excel data — parse columns, detect trends/seasonality/anomalies, forecast 8–12 periods ahead with confidence intervals, and generate a self-contained HTML report with charts.
---

Shared files (written to working directory): `data_profile.json` (column map + stats from Parser), `analysis_results.json` (trend/seasonality/anomalies from Analyzer), `forecast_results.json` (WMA forecast + CI from Forecaster), `report.html` (final output from Visualizer).
...
4. Spawn Forecaster Agent. Wait for completion.
5. Spawn Visualizer Agent. Wait for completion.
6. Read the `summary` field from `analysis_results.json` and `forecast_results.json`. Report key findings to user and confirm `report.html` is ready.
...
## Stop Conditions
- All four agents complete and `report.html` exists → pipeline succeeded
